...
const logOutgoingNotification = async ({ ticketId, customerId, ticketNumber, channel, to, message, subject, ok, externalId, error, }) => {
...
const sendEmailNotification = async ({ to, ticketNumber, }) => {
...
router.post("/website-ticket-notify", requireIntakeSecret, async (req, res) => {
    try {
        const { email_address, phone_number, ticket_number, ticket_id, customer_id, source } = req.body || {};
        ...
        const email = typeof email_address === "string" && email_address.trim().length > 0
            ...
            : null;
        const phoneRaw = typeof phone_number === "string" && phone_number.trim().length > 0
            ? phone_number.trim()
            : null;
        ...
        if (!ticketId || !customerId || !ticketSource) {
            const { data: ticket } = await supabase_1.supabase
                .from("tickets")
        ...
        if (isWebsiteTicket && companyPhone) {
            ...
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : "Company SMS failed";
