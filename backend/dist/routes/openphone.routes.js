...
const fetchBinary = async (url) => {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to download attachment (${response.status}): ${text.slice(0, 140)}`);
    }
...
const uploadToSupabase = async ({ bytes, contentType, fileName, prefix, }) => {
    ...
    const { data: signedUrlData, error: signedError } = await supabase_1.supabase.storage
        .from(bucket)
...
const openPhoneRequest = async (path) => {
...
const extractMessageAttachments = (payload) => {
    const object = payload?.data?.object || payload?.data || payload || {};
    const possible = object.attachments ||
        object.media ||
        object.files ||
        object.images ||
...
router.post("/communications/:id/resync", async (req, res) => {
    try {
        ...
        const { data: communication, error } = await supabase_1.supabase
            .from("communications")
            .select(`
        id,
        ticket_id,
      ...
      `)
        ...
        if (communication.channel === "openphone_call" ||
            ...
            if (typeof callId === "string" && callId.trim().length > 0) {
                ...
                let recordings = null;
                try {
                    recordings = await openPhoneRequest(`/call-recordings/${callId}`);
                ...
                try {
                    const transcript = await openPhoneRequest(`/call-transcripts/${callId}`);
                ...
                try {
                    const summary = await openPhoneRequest(`/call-summaries/${callId}`);
                    const summaryText = (Array.isArray(summary?.data?.summary)
        ...
        if (communication.channel === "openphone_sms" ||
        ...
        if (Object.keys(updates).length > 0) {
            ...
            await supabase_1.supabase.from("communications").update(updates).eq("id", communication.id);
        }
        return res.json({
            success: true,
