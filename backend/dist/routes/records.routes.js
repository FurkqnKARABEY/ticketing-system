...
const isOpenPhoneChannel = (channel) => {
...
const sendTicketCreatedEmail = async ({ to, ticketNumber, }) => {
...
const sendTicketCreatedSms = async ({ to, ticketNumber, }) => {
...
router.post("/:communicationId/add-to-ticket", async (req, res) => {
    try {
        const communicationId = req.params.communicationId;
        if (typeof communicationId !== "string" || !(0, validation_1.isValidUuid)(communicationId)) {
            return res.status(400).json({
                success: false,
      ...
      `)
        ...
        const { error: communicationUpdateError } = await supabase_1.supabase
            .from("communications")
            .update({
            ticket_id: ticket.id,
        })
        ...
        try {
            if (communication.channel === "email") {
                const emailAddress = communication.email_address ||
                ...
                const phoneNumber = communication.phone_number ||
