...
router.get("/:id", async (req, res) => {
    try {
      ...
      `)
        ...
        `)
                ...
                .limit(1)
                .maybeSingle();
            const rawPayload = intakeCommunication?.raw_payload;
            const intakeEmail = typeof intakeCommunication?.email_address === "string" &&
                ...
                : null;
            const intakePhoneFromPayload = rawPayload && typeof rawPayload === "object"
        ...
        `)
                ...
                .maybeSingle();
            const rawPayload = intakeCommunication?.raw_payload;
            const payloadPhone = rawPayload && typeof rawPayload === "object"
                ? (typeof rawPayload.phone === "string" && rawPayload.phone.trim().length > 0
                    ? rawPayload.phone.trim()
                    : typeof rawPayload.phone_number === "string" &&
                        rawPayload.phone_number.trim().length > 0
                        ...
                        : typeof rawPayload.phoneNumber === "string" &&
                            ...
                            : null)
                : null;
            const rawPhone = (typeof intakeCommunication?.phone_number_normalized === "string" &&
                intakeCommunication.phone_number_normalized.trim().length > 0
                ? intakeCommunication.phone_number_normalized.trim()
                : typeof intakeCommunication?.phone_number === "string" &&
                    intakeCommunication.phone_number.trim().length > 0
    ...
    `;
        ...
        let extraCommunications = [];
        if (customer) {
            const email = (typeof customer.email_primary === "string" && customer.email_primary.trim().length > 0
                ...
                : typeof customer.email_secondary === "string" &&
                    ...
                    : null) || null;
            const phone = (typeof customer.phone_primary_normalized === "string" &&
      ...
      `)
        ...
        if (communicationIds.length > 0) {
            const { data: byComm } = await supabase_1.supabase
                .from("attachments")
                .select(`
          id,
          communication_id,
      ...
      `)
        ...
        try {
          ...
          `)
            ...
            const emailAddress = customer?.email_primary ||
                customer?.email_secondary ||
                null;
            const phoneNumber = customer?.phone_primary_normalized ||
                customer?.phone_primary ||
                customer?.phone_secondary_normalized ||
                customer?.phone_secondary ||
                null;
