...
const readOpenPhoneResponse = async (response: Response) => {
  try {
...
router.post("/send-sms", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (ticket_id !== undefined && !isValidUuid(ticket_id)) {
      return res.status(400).json({
        success: false,
    ...
    if (customer_id !== undefined && !isValidUuid(customer_id)) {
      return res.status(400).json({
        success: false,
    ...
    if (!openphoneResponse.ok) {
      ...
      if (openphoneStatus === 429 || openphoneErrorCode === "RATE_LIMIT_EXCEEDED") {
        return res.status(429).json({
          success: false,
          message:
            "OpenPhone rate limit reached. Please wait a few minutes and try again.",
      ...
      if (openphoneStatus === 401 || openphoneStatus === 403) {
        return res.status(openphoneStatus).json({
          success: false,
          message:
            "OpenPhone authorization failed. Please check the OpenPhone API key.",
      ...
      if (openphoneStatus === 400) {
        return res.status(400).json({
          success: false,
          message:
            "OpenPhone rejected the SMS request. Please check the phone number and message body.",
